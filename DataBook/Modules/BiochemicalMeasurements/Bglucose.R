...
bglucose_b5_list_long <- tbls_summary(.mn_pct_md = mn, .variable = b5)
...
bglucose_b5mg_list_long <- tbls_summary(.mn_pct_md = mn, .variable = b5mg)
...
bglucose_ifg_d_list_long <- tbls_summary(.mn_pct_md = pct, .variable = d, .cln = clnall)
...
bglucose_rbg_or_meds_d_list_long <- tbls_summary(.mn_pct_md = pct, .variable = d, .cln = clnall)
...
bglucose_medstext_list_long <- tbls_summary(.mn_pct_md = pct, .variable = medstext, 
...
bglucose_diagn_list_long <- 
  tbls_summary(.data = STEPSClean %>% filter(!is.na(diagn)), .mn_pct_md = pct, .variable = diagn)
