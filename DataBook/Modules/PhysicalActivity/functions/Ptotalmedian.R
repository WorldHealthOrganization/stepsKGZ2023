...
ptotalmedian <- function(.data) {
  
  # variable names that are used in the function
  ptotalmedian_names <- c("sex", "p1", "p2", "p3a", "p3b", "p4", "p5", "p6a", "p6b", 
                          "p7", "p8", "p9a", "p9b", "p10", "p11", "p12a", "p12b", 
                          "p13", "p14", "p15a", "p15b")
  
  # check which names are not in the data before proceeding
  if(!all(i <- rlang::has_name(.data, ptotalmedian_names))) {
    stop(sprintf(
      "%s doesn't contain: %s",
      deparse(substitute(.data)),
      paste(ptotalmedian_names[!i], collapse=", ")))
  } else {
    .data %>%
      mutate(
