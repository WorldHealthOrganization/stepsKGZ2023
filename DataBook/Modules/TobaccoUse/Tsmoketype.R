...
tsmoketype_df <- tsmoketype(datawt)
...
if(.u_r_reg == FALSE) {
  tsmoketype_t5a_list_long_type <- tsmoketype_t5a_list_long %>% mutate(., type = factor("Manufactured cigs"))
  tsmoketype_t5b_list_long_type <- tsmoketype_t5b_list_long %>% mutate(., type = factor("Hand-rolled cigs"))
  tsmoketype_t5c_list_long_type <- tsmoketype_t5c_list_long %>% mutate(., type = factor("Pipes"))
  tsmoketype_t5d_list_long_type <- tsmoketype_t5d_list_long %>% mutate(., type = factor("Cigars, cheerots, cigarillos"))
  tsmoketype_t5e_list_long_type <- tsmoketype_t5e_list_long %>% mutate(., type = factor("Shisha"))
  tsmoketype_t5f_list_long_type <- tsmoketype_t5f_list_long %>% mutate(., type = factor("Heated tobacco products"))
  tsmoketype_t5g_list_long_type <- tsmoketype_t5g_list_long %>% mutate(., type = factor("Other type of tobacco"))
  
  # join all separate tables with full_join from dplyr
  t5a_b <- full_join(tsmoketype_t5a_list_long_type, tsmoketype_t5b_list_long_type)
  t5a_b_c <- full_join(t5a_b, tsmoketype_t5c_list_long_type)
  t5a_b_c_d <- full_join(t5a_b_c, tsmoketype_t5d_list_long_type)
  t5a_b_c_d_e <- full_join(t5a_b_c_d, tsmoketype_t5e_list_long_type)
  t5a_b_c_d_e_f <- full_join(t5a_b_c_d_e, tsmoketype_t5f_list_long_type)
  t5a_b_c_d_e_f_g <- full_join(t5a_b_c_d_e_f, tsmoketype_t5g_list_long_type)
