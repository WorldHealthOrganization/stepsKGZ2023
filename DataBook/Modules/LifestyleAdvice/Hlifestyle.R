...
hlifestyle_df <- hlifestyle(datawt)
...
# Visited a doctor or other health worker during the past 12 months
hlifestyle_v_list_long <- tbls_summary(.mn_pct_md = pct, .variable = v, .cln = h20cln)
# DATABOOK prep
hlifestyle_v <- tbls_list_split(
  .data = hlifestyle_v_list_long, .select_var = v, 
  .vars_amount_number = 2, .select_var_val = "visited a doctor or other health worker")
