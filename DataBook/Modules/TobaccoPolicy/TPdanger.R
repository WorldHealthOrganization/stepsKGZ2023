...
# 5 - extra for KGZ
# Noticed information on the social media channels (e.g., messengers) about dangers of smoking or that encourages quitting
tpdanger_g_list_long <- tbls_summary(.mn_pct_md = pct, .variable = g, .cln = tp1dcln)

# DATABOOK prep
tpdanger_g <- tbls_list_split(
  .data = tpdanger_g_list_long, 
  .select_var = g, .vars_amount_number = 2,
  .select_var_val = "1) noticed information on social media channels")
