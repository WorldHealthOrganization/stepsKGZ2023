...
**Aspirin and statins intake among those diagnosed**

Description: Aspirin and statins intake among those previously diagnosed with raised blood sugar or diabetes.
...
- Are you currently taking statins (Lovastatin/Simvastatin/Atorvastatin or any other statin) regularly to prevent or treat heart disease?

```{r hdiabetes_g_h_tbls}
# column names to use with huxtable
# 1 - Currently taking aspirin among people with diagnosed / history of diabetes
hdiabetes_g_col_names <- c("Age Group (years)", "n", "% Taking aspirin", "95% CI")
hdiabetes_g_reg_col_names <- c("Region", "n", "% Taking aspirin", "95% CI")
# 2 - Currently taking statins among people with diagnosed / history of diabetes
hdiabetes_h_col_names <- c("Age Group (years)", "n", "% Taking statins", "95% CI")
hdiabetes_h_reg_col_names <- c("Region", "n", "% Taking statins", "95% CI")

# men, women, both sexes
# 1 
hdiabetes_g_m_w_b <- join_tbls(hdiabetes_g, m_w_b, 1, hdiabetes_g_col_names)
# 2
hdiabetes_h_m_w_b <- join_tbls(hdiabetes_h, m_w_b, 1, hdiabetes_h_col_names)
```
```{r, eval=.GlobalEnv$.u_r_reg==TRUE}
# urban, rural
# 1
hdiabetes_g_u_r <- join_tbls(hdiabetes_g, u_r, 1, hdiabetes_g_col_names)
# 2
hdiabetes_h_u_r <- join_tbls(hdiabetes_h, u_r, 1, hdiabetes_h_col_names)
```
...
```{r}
# 1
cbind(hdiabetes_g_m_w_b$m, select(hdiabetes_g_m_w_b$w, -agerange), select(hdiabetes_g_m_w_b$b, -agerange)) %>% 
  final_tbl(.disaggregation = m_w_b, .s_amount = 3, .title = "Currently taking aspirin among people with diagnosed / history of diabetes", .colspan_val = 10)
```
...
```{r}
# 2
cbind(hdiabetes_h_m_w_b$m, select(hdiabetes_h_m_w_b$w, -agerange), select(hdiabetes_h_m_w_b$b, -agerange)) %>% 
  final_tbl(.disaggregation = m_w_b, .s_amount = 3, .title = "Currently taking statins among people with diagnosed / history of diabetes", .colspan_val = 10)
```
...
```{r, eval=.GlobalEnv$.u_r_reg==TRUE}
# 1
tbl_m_u_r <- rbind(hdiabetes_g_u_r$m_u, hdiabetes_g_u_r$m_r)
tbl_w_u_r <- rbind(hdiabetes_g_u_r$w_u, hdiabetes_g_u_r$w_r)
tbl_b_u_r <- rbind(hdiabetes_g_u_r$b_u, hdiabetes_g_u_r$b_r)

cbind(tbl_m_u_r, select(tbl_w_u_r, -agerange2), select(tbl_b_u_r, -agerange2)) %>% 
  final_tbl(.disaggregation = u_r, .s_amount = 3, .title = "Currently taking aspirin among people with diagnosed / history of diabetes by type of settlement", .colspan_val = 10)
```
...
```{r, eval=.GlobalEnv$.u_r_reg==TRUE}
# 2
tbl_m_u_r <- rbind(hdiabetes_h_u_r$m_u, hdiabetes_h_u_r$m_r)
tbl_w_u_r <- rbind(hdiabetes_h_u_r$w_u, hdiabetes_h_u_r$w_r)
tbl_b_u_r <- rbind(hdiabetes_h_u_r$b_u, hdiabetes_h_u_r$b_r)

cbind(tbl_m_u_r, select(tbl_w_u_r, -agerange2), select(tbl_b_u_r, -agerange2)) %>% 
  final_tbl(.disaggregation = u_r, .s_amount = 3, .title = "Currently taking statins among people with diagnosed / history of diabetes by type of settlement", .colspan_val = 10)
```
...
```{r, eval=.GlobalEnv$.u_r_reg==TRUE}
# 1
hdiabetes_g$b_reg %>% unite_ci(.ci_amount = 1) %>% apply_hux(.col_names = hdiabetes_g_reg_col_names) %>% 
  final_tbl(reg, 1, "Both sexes", "Currently taking aspirin among people with diagnosed / history of diabetes by region", 4)
```

<br>

```{r, eval=.GlobalEnv$.u_r_reg==TRUE}
hdiabetes_h$b_reg %>% unite_ci(.ci_amount = 1) %>% apply_hux(.col_names = hdiabetes_h_reg_col_names) %>% 
  final_tbl(reg, 1, "Both sexes", "Currently taking statins among people with diagnosed / history of diabetes by region", 4)
```
...
-   Questions used: H6, H7a, H18, H19
-   R script name: Hdiabetes

***


**Aspirin and statins intake among those diagnosed or with raised blood glucose**

Description: Aspirin and statins intake among those previously diagnosed with diabetes or raised blood glucose, or currently on medication for diabetes.
...
```{r hdiabetes_i_j_tbls}
# column names to use with huxtable
# 1 - Currently taking aspirin among diagnosed / history of diabetes or blood glucose>=7 mmol/l or took meds today
hdiabetes_i_col_names <- c("Age Group (years)", "n", "% Taking aspirin", "95% CI")
hdiabetes_i_reg_col_names <- c("Region", "n", "% Taking aspirin", "95% CI")
# 2 - Currently taking statins among diagnosed / history of diabetes or blood glucose>=7 mmol/l or took meds today
hdiabetes_j_col_names <- c("Age Group (years)", "n", "% Taking statins", "95% CI")
hdiabetes_j_reg_col_names <- c("Region", "n", "% Taking statins", "95% CI")

# men, women, both sexes
# 1 
hdiabetes_i_m_w_b <- join_tbls(hdiabetes_i, m_w_b, 1, hdiabetes_i_col_names)
# 2
hdiabetes_j_m_w_b <- join_tbls(hdiabetes_j, m_w_b, 1, hdiabetes_j_col_names)
```
```{r, eval=.GlobalEnv$.u_r_reg==TRUE}
# urban, rural
# 1
hdiabetes_i_u_r <- join_tbls(hdiabetes_i, u_r, 1, hdiabetes_i_col_names)
# 2
hdiabetes_j_u_r <- join_tbls(hdiabetes_j, u_r, 1, hdiabetes_j_col_names)
```
...
```{r}
# 1
cbind(hdiabetes_i_m_w_b$m, select(hdiabetes_i_m_w_b$w, -agerange), select(hdiabetes_i_m_w_b$b, -agerange)) %>% 
  final_tbl(.disaggregation = m_w_b, .s_amount = 3, .title = "Currently taking aspirin among diagnosed / history of diabetes or blood glucose>=7 mmol/l or took medication today", .colspan_val = 10)
```
...
```{r}
# 2
cbind(hdiabetes_j_m_w_b$m, select(hdiabetes_j_m_w_b$w, -agerange), select(hdiabetes_j_m_w_b$b, -agerange)) %>% 
  final_tbl(.disaggregation = m_w_b, .s_amount = 3, .title = "Currently taking statins among diagnosed / history of diabetes or blood glucose>=7 mmol/l or took medication today", .colspan_val = 10)
```
...
```{r, eval=.GlobalEnv$.u_r_reg==TRUE}
# 1
tbl_m_u_r <- rbind(hdiabetes_i_u_r$m_u, hdiabetes_i_u_r$m_r)
tbl_w_u_r <- rbind(hdiabetes_i_u_r$w_u, hdiabetes_i_u_r$w_r)
tbl_b_u_r <- rbind(hdiabetes_i_u_r$b_u, hdiabetes_i_u_r$b_r)

cbind(tbl_m_u_r, select(tbl_w_u_r, -agerange2), select(tbl_b_u_r, -agerange2)) %>% 
  final_tbl(.disaggregation = u_r, .s_amount = 3, .title = "Currently taking aspirin among diagnosed / history of diabetes or blood glucose>=7 mmol/l or took medication today by type of settlement", .colspan_val = 10)
```
...
```{r, eval=.GlobalEnv$.u_r_reg==TRUE}
# 2
tbl_m_u_r <- rbind(hdiabetes_j_u_r$m_u, hdiabetes_j_u_r$m_r)
tbl_w_u_r <- rbind(hdiabetes_j_u_r$w_u, hdiabetes_j_u_r$w_r)
tbl_b_u_r <- rbind(hdiabetes_j_u_r$b_u, hdiabetes_j_u_r$b_r)

cbind(tbl_m_u_r, select(tbl_w_u_r, -agerange2), select(tbl_b_u_r, -agerange2)) %>% 
  final_tbl(.disaggregation = u_r, .s_amount = 3, .title = "Currently taking statins among diagnosed / history of diabetes or blood glucose>=7 mmol/l or took medication today by type of settlement", .colspan_val = 10)
```
...
```{r, eval=.GlobalEnv$.u_r_reg==TRUE}
# 1
hdiabetes_i$b_reg %>% unite_ci(.ci_amount = 1) %>% apply_hux(.col_names = hdiabetes_i_reg_col_names) %>% 
  final_tbl(reg, 1, "Both sexes", "Currently taking aspirin among diagnosed / history of diabetes or blood glucose>=7 mmol/l or took medication today by region", 4)
```

<br>

```{r, eval=.GlobalEnv$.u_r_reg==TRUE}
hdiabetes_j$b_reg %>% unite_ci(.ci_amount = 1) %>% apply_hux(.col_names = hdiabetes_j_reg_col_names) %>% 
  final_tbl(reg, 1, "Both sexes", "Currently taking statins among diagnosed / history of diabetes or blood glucose>=7 mmol/l or took medication today by region", 4)
```
...
-   Questions used: H6, H7a, H18, H19, B5, B6
-   R script name: Hdiabetes
