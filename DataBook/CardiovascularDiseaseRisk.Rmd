...
Description: Proportion of the population aged 40–69 years with a 10-year cardiovascular disease (CVD) risk* ≥30% or with existing CVD.
...
cbind(cvdrisk_c_m_w_b$m, select(cvdrisk_c_m_w_b$w, -1), select(cvdrisk_c_m_w_b$b, -1)) %>% 
  final_tbl(.disaggregation = m_w_b, .s_amount = 3, .title = "Proportion of the population with a 10-year CVD risk ≥30% or with existing CVD", .colspan_val = 10)
```
...
cbind(tbl_m_u_r, select(tbl_w_u_r, -1), select(tbl_b_u_r, -1)) %>% 
  final_tbl(.disaggregation = u_r, .s_amount = 3, .title = "Proportion of the population with a 10-year CVD risk ≥30% or with existing CVD by type of settlement", .colspan_val = 10)
```
...
cvdrisk_c$b_reg %>% unite_ci(.ci_amount = 1) %>% apply_hux(.col_names = cvdrisk_c_reg_col_names) %>% 
  final_tbl(.disaggregation = reg, 1, "Both sexes", "Proportion of the population with a 10-year CVD risk ≥30% or with existing CVD by region", 4)
```
