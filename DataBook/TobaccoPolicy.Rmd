...
- Television
- Radio
- Social media channels (e.g., messengers) / Internet
...
# 4 - Noticed information on social media channels (e.g., messengers) about dangers of smoking or that encourages quitting
tpdanger_g_m_w_b <- join_tbls(tpdanger_g, m_w_b, 1, tpdanger_col_names)
...
tpdanger_g_u_r <- join_tbls(tpdanger_g, u_r, 1, tpdanger_col_names) # 4
```
...
```{r}
# 4
cbind(tpdanger_g_m_w_b$m, select(tpdanger_g_m_w_b$w, -agerange), select(tpdanger_g_m_w_b$b, -agerange)) %>% 
  final_tbl(.disaggregation = m_w_b, .s_amount = 3, .title = "Noticed information on social media channels (e.g., messengers) about dangers of smoking or that encourages quitting", .colspan_val = 10)
```
...
```{r, eval=.GlobalEnv$.u_r_reg==TRUE}
# 4
tbl_m_u_r <- rbind(tpdanger_g_u_r$m_u, tpdanger_g_u_r$m_r)
tbl_w_u_r <- rbind(tpdanger_g_u_r$w_u, tpdanger_g_u_r$w_r)
tbl_b_u_r <- rbind(tpdanger_g_u_r$b_u, tpdanger_g_u_r$b_r)

cbind(tbl_m_u_r, select(tbl_w_u_r, -agerange2), select(tbl_b_u_r, -agerange2)) %>% 
  final_tbl(.disaggregation = u_r, .s_amount = 3, .title = "Noticed information on social media channels (e.g., messengers) about dangers of smoking or that encourages quitting by type of settlement", .colspan_val = 10)
```
...
```{r, eval=.GlobalEnv$.u_r_reg==TRUE}
# 4
tpdanger_g$b_reg %>% unite_ci(.ci_amount = 1) %>% apply_hux(.col_names = tpdanger_reg_col_names) %>% 
  final_tbl(.disaggregation = reg, .s_amount = 1, .s_val = "Both sexes", .title = "Noticed information on social media channels (e.g., messengers) about dangers of smoking or that encourages quitting by region", .colspan_val = 4) 
```
