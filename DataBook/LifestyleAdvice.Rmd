...
Description: Percentage of population who visited a doctor or other health worker in the past 12 months.

Instrument questions:

- During the past 12 months, have you visited a doctor or other health worker?

```{r hlifestyle_v_tbls}
# column names to use with huxtable
hlifestyle_v_col_names <- c("Age Group (years)", "n", "% Visited", "95% CI")
hlifestyle_v_reg_col_names <- c("Region", "n", "% Visited", "95% CI")

# men, women, both sexes
hlifestyle_v_m_w_b <- join_tbls(hlifestyle_v, m_w_b, 1, hlifestyle_v_col_names)
```
```{r, eval=.GlobalEnv$.u_r_reg==TRUE}
# urban, rural
hlifestyle_v_u_r <- join_tbls(hlifestyle_v, u_r, 1, hlifestyle_v_col_names)
```

<br>

```{r}
cbind(hlifestyle_v_m_w_b$m, select(hlifestyle_v_m_w_b$w, -agerange), select(hlifestyle_v_m_w_b$b, -agerange)) %>% final_tbl(.disaggregation = m_w_b, .s_amount = 3, .title = "Visited a doctor or other health worker in the past 12 months", .colspan_val = 10)
```

<br>

```{r, eval=.GlobalEnv$.u_r_reg==TRUE}
tbl_m_u_r <- rbind(hlifestyle_v_u_r$m_u, hlifestyle_v_u_r$m_r)
tbl_w_u_r <- rbind(hlifestyle_v_u_r$w_u, hlifestyle_v_u_r$w_r)
tbl_b_u_r <- rbind(hlifestyle_v_u_r$b_u, hlifestyle_v_u_r$b_r)

cbind(tbl_m_u_r, select(tbl_w_u_r, -agerange2), select(tbl_b_u_r, -agerange2)) %>% 
  final_tbl(.disaggregation = u_r, .s_amount = 3, .title = "Visited a doctor or other health worker in the past 12 months by type of settlement", .colspan_val = 10)
```

<br>

```{r, eval=.GlobalEnv$.u_r_reg==TRUE}
hlifestyle_v$b_reg %>% unite_ci(.ci_amount = 1) %>% apply_hux(.col_names = hlifestyle_v_reg_col_names) %>% final_tbl(reg, 1, "Both sexes", "Visited a doctor or other health worker in the past 12 months by region", 4)
```
...
Description: Percentage of population who received lifestyle advice from a doctor or health worker among those who visited either of them in the past 12 months.

Instrument questions:

- During any of your visits to a doctor or other health worker in the past 12 months, were you advised  to do any of the following:
...
- Reduce sugary beverages in your diet
