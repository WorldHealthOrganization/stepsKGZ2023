...
# - covers CVDrisk.R with only two age ranges (hence adjustable .agerange_var parameter in the function)
...
tbls_summary <- function(.data = STEPSClean, .wt_unwt = wt, .mn_pct_md, .variable, 
  ...
  summary_mn <- function(.data, .variable) {
    srvyr::summarise(
      ...
      mutate(
        ...
        m_low = ifelse(m_low <0, 0, m_low),
        # Convert NA values to 0 in the output for a clean look
        m_low = ifelse(is.na(m_low), 0, m_low),
        m_upp = ifelse(is.na(m_upp), 0, m_upp),
        m_deff = ifelse(is.na(m_deff), 0, m_deff)
      )
  }
  ...
  summary_pct <- function(.data) {
    srvyr::summarise(
      ...
      mutate(
        ...
        across(c(m,m_low,m_upp,m_se), ~100*.x),
        # Convert NA values to 0 in the output for a clean look
        m_low = ifelse(is.na(m_low), 0, m_low),
        m_upp = ifelse(is.na(m_upp), 0, m_upp),
        m_deff = ifelse(is.na(m_deff), 0, m_deff)
  ...
  # Prep passing parameters with or without quotes to tbls_summary() function, using rlang
  ...
  #                                       {{ .cln2 }} == {{ .cln2_val }}, 
  #                                       {{ .cln3 }} == {{ .cln3_val }}) 
  #     else stop("Parameter .mn_pct_md must be specified.")}
  # 2nd version:
  filter_mn_pct_md <- . %>% 
    {if(.mn_pct_md=="mn" | .mn_pct_md=="md") filter(., !is.na({{ .variable }}), 
                                                    {{ .cln }} == {{ .cln_val }}, 
  ...
  #     else stop("Both parameters .wt_unwt and .mn_pct_md must be specified.")}
...
tbls_list_split <- function(.data, .select_var, .vars_amount_number, .select_var_val = FALSE) {
  ...
  # two answers. In this case, select that required answer using the .var_val parameter
...
fs_summary <- function(.data, .sel_var, .name_val, .pct) {
  ...
  if (missing(.pct)) {
    ...
    mutate(m = sprintf("%.1f", m),
           m_low = sprintf("%.1f", m_low),
           m_upp = sprintf("%.1f", m_upp)) %>% 
      ##########################################################################
...
unite_ci <- function(.data, .ci_amount) {
  ##############################################################################
  # Prep passing parameters with or without quotes to unite_ci() function, using rlang
...
join_tbls <- function(.data, .disaggregation, .ci_amount_val, .col_names_val) {
  ##############################################################################
  # Prep passing parameters with or without quotes to join_tbls() function, using rlang
  ...
  # URBAN/RURAL
    ...
    tbls <- list(
      ...
      map(~ unite_ci(., .ci_amount = {{ .ci_amount_val }}) %>% 
            apply_hux(.col_names = {{ .col_names_val }})) %>%
      set_names(u_r)
  } else stop("Both parameters .disaggregation and .ci_amount_val must be specified.")
  return(tbls)
}
...
final_tbl <- function(.data, .disaggregation, .s_amount, .s_val, .title, .colspan_val) {
  ##############################################################################
  # Prep passing parameters with or without quotes to final_tbl() function, using rlang
  .disaggregation <- rlang::quo_name(rlang::enquo(.disaggregation))
  ##############################################################################
  ...
  # Unweighted scripts
    ...
    .data %>% 
      ...
      set_right_border(row = 2, col = 1, value = 0) %>% 
      set_font_size(value = 8)
  } else stop("Both parameters .disaggregation and .s_amount must be specified.")
}
...
forestplot_steps <- function(.multi_vals = FALSE, .pct_mn_md_val, 
  ...
  # Prep passing parameters with or without quotes to forestplot_steps() function, using rlang
